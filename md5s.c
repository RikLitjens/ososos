 ...
 *
 * Contains implementation of the MD5 algorithm:
 *
 * code has been copied from 
 * 		http://www.ccodechamp.com/c-program-of-md5-encryption-algorithm-c-codechamp/
 * yes, there are many remarks to be made about the coding style...
 * resulting MD5 hash code has been validated on
 *		http://onlinemd5.com/
 */
...
md5s (const char * msg, int mlen)
{
	...
	d = md5(msg, mlen);
	// if d[] = { abcd, efgh, ijkl, mnop }
	// then the corresponding 128-bit integer is:
	// 		dcbahgfelkjiponm
	
	// convert the four 32-bit integers into one 128-bit integer
	for (i = 0; i < 4; i++)
	{
		MD5union 	u;
		uint32_t	t = 0;
...
int main( int argc, char *argv[] )
{
    ...
    MD5union u;
    printf("\n\n\nThe MD5 code for input string is (len=%lu): \n", strlen(msg));
    printf("\t= 0x");
    for (j=0;j<4; j++){
        u.w = d[j];
